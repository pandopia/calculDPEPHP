---
section_id: "glossaire-enums"
title: "Mapping enums XSD ↔ valeurs spec"
spec_pages: []
xml_outputs: []
tables: []
depends_on: []
...
# Mapping enums XSD — valeurs textuelles

> Généré depuis `resources/ademe_DPE.xsd` (balises `<xs:appinfo>` JSON).
> Chaque tableau liste les valeurs `id → label` telles que définies dans le XSD.
>
> **Convention inertie** : `enum_classe_inertie_id` utilise la convention LICIEL (1=légère → 4=très lourde),
> à l'inverse de l'ordre textuel du XSD (1=très lourde → 4=légère). Le code PHP suit la convention LICIEL
> pour rester cohérent avec les fichiers verif de référence.

**65 enums recensés.**

## `enum_periode_construction_id`
_Spec : §3 — Enveloppe (périodes)_
...
_Spec : §7 — Inertie thermique (convention LICIEL : 1=légère → 4=très lourde)_
...
_Spec : §3 — Type isolation parois_
...
_Spec : §3.1 — Configuration isolation LNC_
...
## `enum_type_vitrage_id`
_Spec : §3.2.4 — Type vitrage_
...
_Spec : §3.2.4 — Matériaux menuiserie_
...
## `enum_methode_saisie_q4pa_conv_id`
...
## `enum_periode_installation_fr_id`
...
_Spec : Méthode saisie caractéristiques système_
...
## `enum_cfg_installation_ecs_id`
_Spec : §11 — Configuration ECS_
...
_Spec : §9 — Type installation (individuel/collectif)_
...
_Spec : §11 — Installation solaire_
...
_Spec : §11 / §14 — Type générateur ECS_
...
## `enum_equipement_intermittence_id`
_Spec : §8 — Intermittence_
...
_Spec : §9 / §12-13 — Type générateur chauffage_
...
## `enum_usage_fonctionnel_batiment_id`
...
## `enum_methode_application_dpe_ter_id`
...
## `enum_position_etage_logement_id`
...
## `enum_categorie_enr_descriptif_id`
